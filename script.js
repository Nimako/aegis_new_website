...
if (reviewPreview) reviewPreview.innerHTML = '<div class="review-panel-head"><small>REVIEW QUEUE · 1 ACTION</small><strong>Review</strong></div><div class="review-row-focus" role="img" aria-label="Update beneficiary details, needs approval, Review"><span class="review-focus-index">01</span><div class="review-focus-copy"><small>ONE ACTION NEEDS YOUR REVIEW</small><b>Update beneficiary details</b><em>Prepared by Aegis assist · 2m ago</em></div><span class="review-focus-status">Policy check passed</span></div>';
...
const views = {
  ...
  channels: `<div class="channel-grid-ui"><h4 style="grid-column:1/-1">Connect all your channels <span class="badge">4 connected</span></h4>${[['Website','i-channel'],['WhatsApp','i-whatsapp'],['Telegram','i-telegram'],['Voice','i-phone'],['SMS','i-chat'],['API / SDK','i-api']].map(([item, icon], index) => `<div class="channel-option-ui"><span><svg class="icon"><use href="#${icon}"/></svg></span><b>${item}</b><small>${index < 4 ? 'Connected · ready' : 'Set up from dashboard'}</small></div>`).join('')}</div>`,
